...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
            ...
            //---------------------- NO ITEM_MOD AND ORDER_MOD DB VERSION -------------
        // case 'update':
        ...
        //     // Fetch the items associated with the given order_id
        //     $stmt = $conne->prepare("SELECT item_id FROM items WHERE order_id = ?");
        //     $stmt->bind_param('i', $orderId);
        //     $stmt->execute();
        //     $result = $stmt->get_result();

        //     // Collect valid item IDs for the current order
        //     $validItemIds = [];
        //     while ($row = $result->fetch_assoc()) {
        //         $validItemIds[] = $row['item_id'];
        //     }

        //     // Log valid item IDs for debugging
        //     error_log("Valid Item IDs: " . implode(", ", $validItemIds));

        //     // Filter quantities to only include valid item IDs
        //     $filteredQuantities = [];
        //     foreach ($quantities as $itemId => $quantity) {
        //         if (in_array($itemId, $validItemIds)) {
        //             $filteredQuantities[$itemId] = intval($quantity);
        //         }
        //     }

        //     // Log filtered quantities for debugging
        //     error_log("Filtered Quantities: " . json_encode($filteredQuantities));

        //     // Check if there are any quantities left to update
        //     if (empty($filteredQuantities)) {
        //         echo json_encode(['error' => 'No valid quantities to update.']);
        //         exit;
        //     }

        //     // Proceed with updating the quantities if valid
        //     foreach ($filteredQuantities as $itemId => $quantity) {
